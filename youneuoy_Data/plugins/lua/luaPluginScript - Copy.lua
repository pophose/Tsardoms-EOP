...
local titlesGrant = false
local openAncList = false
local selectedGeneral = {}
local selectedgenanc = {}
...
-- COPY FOR TITLES GRANTER
        ImGui.SetNextWindowSize(76, 59)
        ImGui.SetNextWindowPos(1382, 755)
        OpenDCButton, shouldDraw = ImGui.Begin("Granter##ButtonMain", OpenDCButton,
            bit.bor(ImGuiWindowFlags.NoDecoration, ImGuiWindowFlags.NoMove, ImGuiWindowFlags.NoBackground,
                ImGuiWindowFlags.NoScrollWithMouse))

            tsCustomButton("##GranterButtonMain", 0, 0, 77, 57,
                function()
                    titlesGrant = not titlesGrant
                end,
                ScrollOpenSound, DCScrollButton)
    ...
    elseif(titlesGrant) then
        --Main Window
        ImGui.SetNextWindowSize(980, 835)
        ImGui.SetNextWindowPos(1, -3    )
        titlesGrant, shouldDraw = ImGui.Begin("Granter##GTMain", titlesGrant,
            bit.bor(ImGuiWindowFlags.NoDecoration, ImGuiWindowFlags.NoMove, ImGuiWindowFlags.NoBackground, ImGuiWindowFlags.NoScrollWithMouse))
        ...
        ImGui.SetCursorPos(65, 144)
        ImGui.SetNextWindowBgAlpha(0.0)
        local shouldDraw = ImGui.BeginChild("Granter##GenrlListWindow", 815, 580,
            bit.bor(ImGuiWindowFlags.NoDecoration, ImGuiWindowFlags.NoMove, ImGuiWindowFlags.NoBackground))


        if openAncList then
            ImGui.LabelText(selectedGeneral.namedCharacter.fullName)
            ImGui.Image(selectedgenanc.id, selectedgenanc.x, selectedgenanc.y)
        end
        --Iterates through all living generals saved to the table
        for i= 1, #campaignEOPData.generals  do
            local thisGeneral = campaignEOPData.generals[i]
            ImGui.PushStyleColor(ImGuiCol.Text, 0.0, 0.0, 0.0, 1.0) --make text black
            ImGui.SetWindowFontScale(1.3) -- enlarge text size
            ImGui.LabelText(thisGeneral.namedCharacter.fullName)
            ImGui.SetWindowFontScale(1.0) -- normalize text size

            ImGui.SetCursorPos(20
            , ImGui.GetCursorPosY())
            if  campaignEOPData.genportraits then 
                ImGui.Image(campaignEOPData.genportraits[i].id, campaignEOPData.genportraits[i].x, campaignEOPData.genportraits[i].y)       --Loads the general's portrait for display
                ImGui.SetCursorPos(25+campaignEOPData.genportraits[i].x, ImGui.GetCursorPosY()-campaignEOPData.genportraits[i].y)           
                ImGui.Text("Command:"..thisGeneral.namedCharacter.command)                                                                  --Displays the character's stats (command, chivalry/dread, etc.) Currently only in text
                ImGui.SetCursorPos(25+campaignEOPData.genportraits[i].x, ImGui.GetCursorPosY())
                ImGui.Text("Authority:"..thisGeneral.namedCharacter.authority)
                ImGui.SetCursorPos(25+campaignEOPData.genportraits[i].x, ImGui.GetCursorPosY())
                ImGui.Text("Chiv/Dread:"..thisGeneral.namedCharacter.chivalryAndDread)
                ImGui.SetCursorPos(25+campaignEOPData.genportraits[i].x, ImGui.GetCursorPosY())
                ImGui.Text("Loyalty:"..thisGeneral.namedCharacter.loyalty)
            end
            ImGui.SetWindowFontScale(1.1) -- enlarge text size
            ImGui.SetCursorPos(25, ImGui.GetCursorPosY()+35)
            ImGui.TextWrapped("Grant fiefs to this general")
            ImGui.SetWindowFontScale(1.0) -- normalize text size

            ImGui.SetCursorPos(480, (ImGui.GetCursorPosY())+10)
            ImGui.PushStyleColor(ImGuiCol.Text, 0.05, 0.4, 0.05, 1.0)
            ImGui.TextWrapped("Grant fief? Y/N")
            ImGui.PopStyleColor(2)            -- removes text assigned colour from stack(imgui logic)


            tsCustomButton("##GrantButton" .. i, 720, ImGui.GetCursorPosY()-64, 64, 64,          --the "i" in the button name ensures that every button has a unique name, so it can function!
                function()
                    openAncList = not openAncList
                    titlesGrant = not titlesGrant
                    selectedGeneral = thisGeneral
                    selectedgenanc = campaignEOPData.genportraits[i]
                end,
                ButtonSound, greekButton)
        ...
        --Exit button, without texture in this instance because it needed to be bigger than the button
        tsCustomButton("##GrantMainExitButton", 835, 740, 100, 80,          
            function()
                titlesGrant = false;
            end,
            ButtonSound, nil)

    --[[ elseif(openAncList) then                    --titlesList Window
        --Main Decision Window
        ImGui.SetNextWindowSize(980, 835)
        ImGui.SetNextWindowPos(1, -3    )
        openAncList, shouldDraw = ImGui.Begin("Granter##GTMain", openAncList,
            bit.bor(ImGuiWindowFlags.NoDecoration, ImGuiWindowFlags.NoMove, ImGuiWindowFlags.NoBackground, ImGuiWindowFlags.NoScrollWithMouse))
        ...
        ImGui.PushFont(_ScrollTitleFont)
        ImGui.PushStyleColor(ImGuiCol.Text, 0.0, 0.0, 0.0, 1.0) --make text black
        ImGui.Text("Granting Fief to:"..selectedGeneral.namedCharacter.fullName)
        ImGui.PopStyleColor()            -- removes text assigned colour from stack(imgui logic)
        ImGui.PopFont()                  -- no argument needed
        ImGui.SetWindowFontScale(1.0)     -- normalize text size
        ...
        ImGui.SetCursorPos(65, 144)
        ImGui.SetNextWindowBgAlpha(0.0)
        local shouldDraw = ImGui.BeginChild("Granter##TitleListWindow", 815, 580,
            bit.bor(ImGuiWindowFlags.NoDecoration, ImGuiWindowFlags.NoMove, ImGuiWindowFlags.NoBackground))



        --decisions are part of the DCListWindow, divided by separators
        for i= 1, #campaignEOPData.settlements  do
            local thisSettlement = campaignEOPData.settlements[i]
            ImGui.PushStyleColor(ImGuiCol.Text, 0.0, 0.0, 0.0, 1.0) --make text black
            ImGui.SetWindowFontScale(1.3) -- enlarge text size
            ImGui.LabelText(thisSettlement.name)
            ImGui.SetWindowFontScale(1.0) -- normalize text size
            ...
            ImGui.SetWindowFontScale(1.1) -- enlarge text size
            ImGui.SetCursorPos(25, ImGui.GetCursorPosY()+35)
            local thisAnc = ancTable[PlayerFaction.name][thisSettlement.name]
            if thisAnc then
                ImGui.TextWrapped(thisAnc.anc)    
                ImGui.SetWindowFontScale(1.0) -- normalize text size
                if campaignEOPData.ancTga[i] then
                    ImGui.Image(campaignEOPData.ancTga[i].id,campaignEOPData.ancTga[i].x,campaignEOPData.ancTga[i].y)
                end
                ImGui.SetCursorPos(480, (ImGui.GetCursorPosY())+10)
                ImGui.PushStyleColor(ImGuiCol.Text, 0.05, 0.4, 0.05, 1.0)
                ImGui.TextWrapped("Grant fief? Y/N")
                ...
                tsCustomButton("##GrantButtonT" .. i, 720, ImGui.GetCursorPosY()-64, 64, 64,          --the "i" in the button name ensures that every button has a unique name, so it can function!
                    function()
                        --openAncList = not openAncList
                    
                        print("Granted fief to general:".. selectedGeneral.namedCharacter.fullName)
                        selectedGeneral.namedCharacter:addAncillary(ancTable[PlayerFaction.name][thisSettlement.name].anc)
                    end,
                    ButtonSound, greekButton)
