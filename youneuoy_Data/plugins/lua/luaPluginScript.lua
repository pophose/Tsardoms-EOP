...
_ScrollTitleFont = ImGui.GetIO().Fonts:AddFontFromFileTTF(M2TWEOP.getModPath().."\\data\\fonts\\Kingthings_Petrock_light.ttf", 18, nil, nil)
end
...
function draw(pDevice)
    ...
    if (OpenDCMain) then
        ...
        ImGui.SetNextWindowSize(950, 835)
        ...
        ImGui.SetWindowFontScale(2.6)     -- enlarge text size
        ...
        for i, decision in ipairs(ActiveDecisions) do
            ...
            ImGui.SetCursorPos(480, ImGui.GetCursorPosY())
            ...
            ImGui.SetCursorPos(720, ImGui.GetCursorPosY())          --enact button
            pushTransparentButtonStyle()
            if ImGui.ImageButton("##DCEnactButton" .. i, greekButton.img, 64, 64) then
        ...
        --exit button texture
        ImGui.SetCursorPos(820, 720)
        xPos, yPos = ImGui.GetCursorPos()
        ImGui.Image(greekButton.img, 130, 110)
        ImGui.SetCursorPos(xPos, yPos)

        --exit button
        ImGui.PushStyleColor(ImGuiCol.Button, 0, 0, 0, 0)               -- Transparent button background
        ImGui.PushStyleColor(ImGuiCol.ButtonHovered, 1, 1, 1, 0.08)     -- Transparent when hovered
        ImGui.PushStyleColor(ImGuiCol.ButtonActive, 0, 0, 0, 0.1)         -- Transparent when clicked
        ImGui.PushStyleVar(ImGuiStyleVar.FramePadding, 0, 0)            -- Remove padding
        ImGui.SetCursorPos(835, 735)
        if ImGui.Button("##DCMainExitButton", 100, 80) then                  --generate button, and add functionality
